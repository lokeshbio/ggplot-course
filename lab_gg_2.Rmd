...
We can create subplots using the facetting functionality.
...
If we try the same with the gene counts data facetted by time.
...
There are a couple of things to note here. In the above example, we use the pipe `%>%` symbol that redirects the output of one command as the input to another. Then we group the data by the variable `Time`, followed by summarising the `count`  with `mean()` and `sd()` functions to get the mean and standard devialtion of their respective counts. The `head()` function just prints the first few lines.

Now that we have summarised the data to be bale to plot the bar graph that we want, we can just input the data to ggplot as well using the `%>%` sign. 
...
Let's now try to make stacked bars. For this let's try to make the data more usable for stacked bars. For this let's use the `greoup_by` function to make the groups based on both `Time` and `Replicate`.
...
We can try now to plot error bars on them. This gets a bit complicated as the error
...
You can also make these error bars look nicer by playing aroud with some of the parameters available, like example below:
