...
<i class="fas fa-lightbulb"></i> Tip &nbsp; `geom_line()` is a bit tricky when you use it together with `groups`. It tries to draw lines within the group. In this case, if you would like to draw lines between the groups (like in the above plot, between `t0` through `t2` to `t24`), you initate the ggplot with aesthetics for the line and add `geom_line(aes(group=1))` this way. 

<i class="fas fa-lightbulb"></i> Tip &nbsp; This figure has `theme_light()`
