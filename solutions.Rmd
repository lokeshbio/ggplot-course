...
gene_counts_all <- 
  gc_raw %>% rownames_to_column(var = "Gene") %>% gather(Sample_ID, Raw, -Gene) %>%
  full_join(gc_filt %>% rownames_to_column(var = "Gene") %>% gather(Sample_ID, Filtered, -Gene), by = c("Gene", "Sample_ID")) %>%
  full_join(gc_vst %>% rownames_to_column(var = "Gene") %>% gather(Sample_ID, VST, -Gene), by = c("Gene", "Sample_ID")) %>%
  full_join(gc_deseq %>% rownames_to_column(var = "Gene") %>% gather(Sample_ID, DESeq2, -Gene), by = c("Gene", "Sample_ID")) %>%
