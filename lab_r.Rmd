...
<i class="fas fa-clipboard-list"></i> Task &nbsp; Here in this exercise we have used the `counts_raw.txt` file, you can try to make similar R objects for each of the other three counts (`count_filtered.txt`, `counts_vst.txt` and `counts_deseq2.txt`) in `long format`. So for example you would have `gc_filt`, `gc_vst` and `gc_deseq2` R objects in the end.
